...
class QueryUtils {
    ...
    static List<NewsData> fetchNewsData(String queryUrl, Context context) {
    ...
    private static List<NewsData> extractNews(String jsonResponse, Context context) {
            ...
            if (jsonObject.has(context.getString(R.string.response_object))) {
                ...
                if (responseJSONObject.has(context.getString(R.string.results_array))) {
                    ...
                    for (int i = 0; i < resultsJSONArray.length(); i++) {
                        ...
                        String authors;
                        if (currentJSONObject.has(context.getString(R.string.tags))) {
                            JSONArray currentFirst = currentJSONObject.getJSONArray(context.getString(R.string.tags));
                            if (currentFirst.length() >= 1) {
                                JSONObject c = currentFirst.getJSONObject(0);
                                authors = c.getString(context.getString(R.string.autherWebTittle));
                            } else {
                                authors = null;
                            }
                        } else {
                        ...
                        newsData.add(new NewsData(title, section, publicationDateTime, webUrl, thumbnail, authors));
                    }
                }
            }
